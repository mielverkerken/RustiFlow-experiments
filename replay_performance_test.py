...
exporters = {
    ...
    "argus": {
        "name": "argus",
        "shell": True,
        "cmd": "/users/mverkerk/RustiFlow-experiments/argus_script_realtime.sh {interface} {interface}_argus.csv > {output_folder}/argus_realtime.log 2>&1",
        "cwd": None,  # Runs from any directory
    },
...
class Experiment:
    ...
    def run(self):
        exporter_command = exporters[self.extractor]["cmd"].format(
            interface=self.interface, output_folder=self.folder
        )
